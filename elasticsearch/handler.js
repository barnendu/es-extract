...
var scroll_id, scroll_size,dataCount;
 ...
 var query={"query": { "match_all": {}}, "size": config.scroll_length};
  ...
  dataCount=0;
   var options = {
        method: 'POST',
        url: 'http://' + config.source_server + '/'+ config.index +'/' + config.type + '/_search?search_type=scan&scroll=3m',
...
function fetchNextDataSet(type){
                ...
                if(scroll_size)
                updateDataSet(config.type,JSON.parse(res.getBody('utf8')).hits.hits);
...
_.each(data,function(rslt,ind){
    if(!_.isEmpty(rslt._source)){
        if(!query){
        query =JSON.stringify({"create":{"_id" :rslt._id , "_type" : type, "_index" : index}})+"\n" +
       JSON.stringify(JSON.parse(JSON.stringify(rslt._source).replace(/"\s+|\s+"/g,'"')))+"\n" ;
        }else{
       query +=JSON.stringify({"create":{"_id" :rslt._id , "_type" : type, "_index" : index}})+"\n" +
       JSON.stringify(JSON.parse(JSON.stringify(rslt._source).replace(/"\s+|\s+"/g,'"')))+"\n" ;
        }
